...
async function 反代参数获取(url, uuid, 默认反代IP = '', 默认反代兜底 = true) {
	...
	保存快照();	// 如果反代IP已设置（来自环境变量 PROXYIP 或参数 proxyip），且没有其他代理类型，则自动启用 proxyip 代理模式
	// 这样所有请求都会先走 PROXYIP，跳过直连
	if (反代IP && 反代IP !== 'auto' && !启用SOCKS5反代) {
		启用SOCKS5反代 = 'proxyip';
		启用SOCKS5全局反代 = true;
	}
	return 反代上下文;
